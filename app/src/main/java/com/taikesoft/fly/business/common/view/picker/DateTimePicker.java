package com.taikesoft.fly.business.common.view.picker;
...
import com.taikesoft.fly.business.common.view.WheelView;
...
public class DateTimePicker extends WheelPicker {
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        TLog.log("setSelectedItem --> change months and days while set selected");
    ...
    public void setSelectedItem(int yearOrMonth, int monthOrDay, int hour, int minute) {
        ...
        if (dateMode == MONTH_DAY) {
            ...
            TLog.log("setSelectedItem --> change months while set selected");
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) && years.size() == 0) {
            TLog.log("makeCenterView --> init years before make view");
            initYearData();
        }
        if (dateMode != NONE && months.size() == 0) {
            TLog.log("makeCenterView --> init months before make view");
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            TLog.log("makeCenterView --> init days before make view");
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
        ...
        if (timeMode != NONE && hours.size() == 0) {
            TLog.log("makeCenterView --> init hours before make view");
            initHourData();
        }
        if (timeMode != NONE && minutes.size() == 0) {
            TLog.log("makeCenterView --> init minutes before make view");
        ...
        if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
            ...
            yearView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    TLog.log("onSelected --> change months after year wheeled");
                    if (resetWhileWheel) {
        ...
        if (dateMode != NONE) {
            ...
            monthView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
                        TLog.log("onSelected --> change days after month wheeled");
                        if (resetWhileWheel) {
                            selectedDayIndex = 0;//重置日子索引
                        }
                        int selectedYear;
        ...
        if (timeMode != NONE) {
            ...
            hourView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    TLog.log("onSelected --> change minutes after hour wheeled");
                    changeMinuteData(DateUtils.trimZero(selectedHour));
    ...
    private void changeMonthData(int selectedYear) {
        ...
        if (!resetWhileWheel) {
            ...
            TLog.log("changeMonthData --> " + preSelectMonth);
        }
        months.clear();
    ...
    private void changeDayData(int selectedYear, int selectedMonth) {
        ...
        if (!resetWhileWheel) {
            ...
            TLog.log("changeDayData --> maxDays=" + maxDays + ", preSelectDay=" + preSelectDay);
        }
        days.clear();
